    ...
    public partial class AddForm2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string sql = "SELECT add_worker( @name, @id, @number, @id_service, @login, @pass, @role::role_type);";
                    using (var cmd = new NpgsqlCommand(sql, conn))
                    {
                        ...
                        MessageBox.Show("Работник успешно добавлен в базу данных!");
                    }
                }
            }
            catch (Exception ex)
            {
                // Обработка ошибок
                MessageBox.Show($"Ошибка при добавлении работника: {ex.Message}");
            }
        }
    }
}
